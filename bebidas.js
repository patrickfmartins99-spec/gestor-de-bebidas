document.addEventListener('DOMContentLoaded', () => {
    ...
    const calcularGastoEstimado = (estoqueAtual, ultimoEstoque) => {
        return estoqueAtual < ultimoEstoque ? ultimoEstoque - estoqueAtual : 0;
    };

    const calcularReposicaoEstimada = (estoqueAtual, ultimoEstoque) => {
        return estoqueAtual > ultimoEstoque ? estoqueAtual - ultimoEstoque : 0;
    };
    ...
    const gerarRelatorioPDFBebidas = (contagem, filenamePrefix) => {
        ...
        Object.keys(contagem.detalhesContagem).forEach(bebidaId => {
            ...
            if (bebidaInfo.unidade === 'caixa' || bebidaInfo.unidade === 'fardo') {
                depositoTexto = `${dados.deposito} ${bebidaInfo.unidade}(s) e ${dados.unidadesAvulsas} unidade(s)`;
            } else {
                depositoTexto = `${dados.deposito} unidade(s)`;
            }
            
            const gastoReposicao = dados.gastoEstimado > 0 ? `-${dados.gastoEstimado} (Gasto)` : `+${dados.reposicaoEstimada} (Reposição)`;
            ...
            tabelaRows += `
                <tr>
                    ...
                    <td style="padding: 10px; border: 1px solid #000; text-align: center;">${gastoReposicao}</td>
                </tr>
            `;
        });
                ...
                <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 14px;">
                    <thead>
                        <tr style="background-color: #333; color: white;">
                            ...
                            <th style="padding: 12px; border: 1px solid #000; text-align: center;">GASTO/REPOSIÇÃO</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${tabelaRows}
    ...
    const gerarRelatorioEstoqueAtualBebidas = () => {
        ...
        bebidas.forEach(bebida => {
            ...
            tabelaRows += `
                <tr>
                    <td style="padding: 8px; border: 1px solid #000; font-size: 12px;">${bebida.nome}</td>
                    <td style="padding: 8px; border: 1px solid #000; font-size: 12px;">${bebida.unidade}</td>
                    <td style="padding: 8px; border: 1px solid #000; font-size: 12px;">${totalUnidades}</td>
                </tr>
            `;
        });
        ...
        const conteudoRelatorio = `
            <div style="font-family: Arial, sans-serif; padding: 2rem;">
                ...
                <table style="width: 100%; border-collapse: collapse; margin-top: 1rem; border: 1px solid #000;">
                    <thead>
                        <tr style="background-color: #333; color: white;">
                            <th style="padding: 8px; border: 1px solid #000; text-align: left; font-size: 12px;">BEBIDA</th>
        ...
        const options = {
            margin: 1,
            filename: `Relatorio_Estoque_Atual_Bebidas_${dataAtual}.pdf`,
            image: { type: 'jpeg', quality: 0.98 },
            html2canvas: { scale: 2 },
            jsPDF: { unit: 'in', format: 'letter', orientation: 'portrait' }
        };
    ...
    // --- LÓGICA DE PÁGINAS ---
    // Lógica para a tela de Contagem de Bebidas (index.html)
