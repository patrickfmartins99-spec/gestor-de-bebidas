document.addEventListener('DOMContentLoaded', () => {
    ...
    const calcularTotalUnidades = (fardos, unidadesAvulsas, unidadesPorFardo) => {
        if (unidadesPorFardo === 1) { // Para itens contados por unidade
            return fardos + unidadesAvulsas;
        }
        return (fardos * unidadesPorFardo) + unidadesAvulsas;
    };
    ...
    const gerarRelatorioPDFBebidas = (contagem, filenamePrefix) => {
        ...
        Object.keys(contagem.detalhesContagem).forEach(bebidaId => {
            const bebidaInfo = getBebidas().find(b => b.id === bebidaId) || { nome: 'Desconhecida', unidade: 'N/A' };
            const dados = contagem.detalhesContagem[bebidaId];
            
            tabelaRows += `
                <tr>
                    <td style="padding: 10px; border: 1px solid #000;">${bebidaInfo.nome}</td>
                    <td style="padding: 10px; border: 1px solid #000; text-align: center;">${dados.deposito}</td>
                    <td style="padding: 10px; border: 1px solid #000; text-align: center;">${dados.unidadesAvulsas}</td>
                ...
                <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 14px;">
                    <thead>
                        <tr style="background-color: #333; color: white;">
                            ...
                            <th style="padding: 12px; border: 1px solid #000; text-align: center;">DEPÓSITO (FARDO/CAIXA)</th>
                            <th style="padding: 12px; border: 1px solid #000; text-align: center;">UNIDADES AVULSAS</th>
            ...
            ordemCategorias.forEach(categoria => {
                if (bebidasPorCategoria[categoria]) {
                    ...
                    bebidasPorCategoria[categoria].forEach(bebida => {
                        ...
                        const ultimaContagemData = ultimaContagem?.data;

                        let camposDeposito = '';
                        if (bebida.unidade === 'caixa' || bebida.unidade === 'fardo') {
                            camposDeposito = `
                                <div class="col-md-3">
                                    <label class="form-label">Depósito (${bebida.unidade})</label>
                                    <input type="number" step="any" class="form-control form-control-sm" data-campo="deposito-${bebida.unidade}" placeholder="Qtd. ${bebida.unidade}">
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label">Unidades Avulsas</label>
                                    <input type="number" step="any" class="form-control form-control-sm" data-campo="unidades-avulsas" placeholder="Qtd. Unidades">
                                </div>
                            `;
                        } else {
                             camposDeposito = `
                        ...
                        bebidaDiv.innerHTML = `
                            <h5 class="bebida-nome">${bebida.nome} <span class="badge bg-primary ms-2">${bebida.unidade}</span></h5>
                            ...
                            <div class="row g-2 align-items-end">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <input type="number" step="any" class="form-control form-control-sm" data-campo="freezer" placeholder="Qtd. Freezer">
                                </div>
                                <div class="col-md-3">
                        ...
                        inputs.forEach(input => {
                            input.addEventListener('input', () => calcularValoresBebida(bebidaDiv, ultimoTotalUnidades, bebida.unidadePorFardo));
                        });

                        calcularValoresBebida(bebidaDiv, ultimoTotalUnidades, bebida.unidadePorFardo);
                    });
                }
            });
        };

        const calcularValoresBebida = (bebidaDiv, ultimoTotalUnidades, unidadesPorFardo) => {
            let depositoFardos = 0;
            let unidadesAvulsas = 0;
            
            if (unidadesPorFardo > 1) {
                depositoFardos = parseFloat(bebidaDiv.querySelector('[data-campo^="deposito-"]').value) || 0;
                unidadesAvulsas = parseFloat(bebidaDiv.querySelector('[data-campo="unidades-avulsas"]').value) || 0;
            } else {
                unidadesAvulsas = parseFloat(bebidaDiv.querySelector('[data-campo="unidades-avulsas"]').value) || 0;
            }
            ...
            if (totalUnidades > ultimoTotalUnidades) {
                gastoReposicaoSpan.textContent = `+${totalUnidades - ultimoTotalUnidades} (Reposição)`;
                gastoReposicaoSpan.style.color = 'green';
            } else if (totalUnidades < ultimoTotalUnidades) {
                gastoReposicaoSpan.textContent = `${totalUnidades - ultimoTotalUnidades} (Gasto)`;
        ...
        formContagemBebidas.addEventListener('submit', (event) => {
            ...
            bebidasNaTela.forEach(bebidaDiv => {
                ...
                if (bebidaInfo.unidade === 'caixa' || bebidaInfo.unidade === 'fardo') {
                    depositoValor = parseFloat(bebidaDiv.querySelector('[data-campo^="deposito-"]').value) || 0;
                        ...
                        if (bebida.unidade === 'caixa' || bebida.unidade === 'fardo') {
                            camposContagem = `
                                <div class="col-6 col-md-3">
                                    ...
                                    <input type="number" step="any" class="form-control form-control-sm" data-campo="deposito-fardos" placeholder="Qtd. Fardos">
                                </div>
                                <div class="col-6 col-md-3">
                                    <label class="form-label">Unidades Avulsas</label>
                                    <input type="number" step="any" class="form-control form-control-sm" data-campo="unidades-avulsas" placeholder="Qtd. Unidades">
